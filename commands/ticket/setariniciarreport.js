...
module.exports = {
    ...
    async execute(client, message, args) {
        const $o = async () => {
            ...
            if (!id) {
                message.delete();
                message.channel.send(`Coloque o ID do canal de mensagens de report.`);
            } else {
                db.set(`IniciarReport_${message.guild.id}`, id);
                message.delete();
                message.channel.send(`O inicar de report foi setado nesse ID: ${id}`);
            }
        };
